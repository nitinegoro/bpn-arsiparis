...
class Trash
{
	...
	private function _all()
	{
		...
		$row = $this->ci->db->query("SELECT tb_buku_tanah.*, tb_warkah.* FROM tb_buku_tanah JOIN tb_warkah ON tb_buku_tanah.no208 = tb_warkah.no208 WHERE tb_buku_tanah.id_bukutanah = '{$this->id_bukutanah}'")->row();
		// Hapus File Dokumen
		$data_file_buku = $this->ci->db->query("SELECT * FROM tb_file_buku WHERE id_bukutanah = '{$this->id_bukutanah}'")->result();
		...
		$this->ci->db->where('id_bukutanah', $this->id_bukutanah);
