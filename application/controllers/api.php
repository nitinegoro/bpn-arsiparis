...
class Api extends CI_Controller {
	...
	public function cek_buku( $lemari = 0, $rak = 0, $album = 0, $halaman = 0)
	{
		 ...
		 * @return string (Object)
		 **/
		$row = $this->db->query("SELECT tb_simpan_buku.*, tb_buku_tanah.* FROM tb_simpan_buku INNER JOIN tb_buku_tanah ON tb_simpan_buku.id_bukutanah = tb_buku_tanah.id_bukutanah WHERE tb_simpan_buku.no_lemari = '{$lemari}' AND tb_simpan_buku.no_rak = '{$rak}' AND tb_simpan_buku.no_album = '{$album}' AND tb_simpan_buku.no_halaman = '{$halaman}'")->row();
		if(!$row) :
	...
	public function cek_warkah( $lemari = 0, $rak = 0, $album = 0, $halaman = 0)
	{
		 ...
		 * @return string (Object)
		 **/
		$row = $this->db->query("SELECT tb_simpan_warkah.*, tb_warkah.*, tb_buku_tanah.* FROM tb_simpan_warkah JOIN tb_warkah ON tb_simpan_warkah.id_warkah = tb_warkah.id_warkah JOIN  tb_buku_tanah ON tb_warkah.id_bukutanah =  tb_buku_tanah.id_bukutanah WHERE tb_simpan_warkah.no_lemari = '{$lemari}' AND tb_simpan_warkah.no_rak = '{$rak}' AND tb_simpan_warkah.no_album = '{$album}' AND tb_simpan_warkah.no_halaman = '{$halaman}'")->row();
		if(!$row) :
			$output = array('status' => false, );
		else :
			$output = array(
	...
	{
		$row = $this->db->query("SELECT tb_buku_tanah.*, tb_simpan_buku.*, tb_warkah.* FROM tb_buku_tanah INNER JOIN tb_simpan_buku ON tb_buku_tanah.id_bukutanah = tb_simpan_buku.id_bukutanah JOIN tb_warkah ON tb_buku_tanah.no208 = tb_warkah.no208 WHERE tb_buku_tanah.id_bukutanah = '{$ID}'")->row();
		if(!$row) :
			$output = array('status' => false, );
		else :
			$output = array(
