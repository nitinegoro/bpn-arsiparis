...
class Utama extends CI_Controller {
	...
	public function index()
	{
		$data = array(
			...
			'history' => $this->m_users->getHistoryLimit($this->session->userdata('login')['nip']),
		);
	...
	public function get_document( )
	{
		...
		for($b=$tahun; $b<=date('Y'); $b++) {  
			$output['datasets'][0]['data'][] = $this->m_presentation->getDok_tahun($b);
		}
...
/*		$tahun = (int)date('Y') - 4;
	...
	public function get_districts()
	{
		...
		foreach ($this->m_apps->kecamatan() as $row) :
			...
			$output[] = array(
				'value' => $this->m_presentation->getDok_luas($row->id_kecamatan),
	...
	{
		/**
		 * Menampilkan Data Jenis Hak pada Daigram Pie
		 *
		 * @return String (Object) // Liat Hasil buka program ini dengan JSON_PRETTY_PRINT
		 **/
		$output = array();
		...
		foreach ($this->mbpn->jenis_hak() as $row) :
			if($row->id_hak == 5) : continue; endif;
			$output[] = array(
				'value' => $this->m_presentation->numJenishak($row->id_hak),
				'color' => colorChart($row->id_hak),
				'highlight' => colorChart(--$no),
				'label' => $row->jenis_hak,
			);
		endforeach;
