...
class M_warkah extends CI_Model {
	...
	public function peminjaman(Array $data, $limit = 20, $offset = 0)
	{
		...
		$this->db->join('tb_buku_tanah', 'tb_warkah.id_bukutanah = tb_buku_tanah.id_bukutanah');
		$this->db->join('tb_users', 'tb_pinjam_warkah.nip = tb_users.nip');
		...
		if ($data['petugas'] != '') $this->db->where('tb_pinjam_warkah.nip', $data['petugas']);
	...
	public function laporan_pinjam($bln = 0, $thn = 0)
	{
		$query = $this->db->query("SELECT tb_pinjam_warkah.*, tb_warkah.*, tb_buku_tanah.*, tb_hak_milik.*, tb_users.* FROM tb_pinjam_warkah 
			JOIN tb_warkah ON tb_pinjam_warkah.id_warkah = tb_warkah.id_warkah JOIN tb_buku_tanah ON tb_warkah.id_bukutanah = tb_buku_tanah.id_bukutanah JOIN tb_hak_milik ON tb_buku_tanah.id_hak = tb_hak_milik.id_hak JOIN tb_users ON tb_pinjam_warkah.nip = tb_users.nip WHERE tb_pinjam_warkah.bulan = '{$bln}' AND tb_pinjam_warkah.tahun = '{$thn}'");
		return $query->result();
	}
}
