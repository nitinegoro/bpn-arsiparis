...
class Pinjam_warkah extends CI_Controller {
	...
	public function cetak($ID=0)
	{
		$query = $this->db->query("SELECT tb_pinjam_warkah.*, tb_warkah.*, tb_buku_tanah.*, tb_users.* FROM tb_pinjam_warkah 
		 JOIN tb_warkah ON tb_pinjam_warkah.id_warkah = tb_warkah.id_warkah JOIN tb_buku_tanah ON tb_warkah.id_bukutanah = tb_buku_tanah.id_bukutanah JOIN tb_users ON tb_pinjam_warkah.nip = tb_users.nip WHERE tb_pinjam_warkah.id_pinjam_warkah = '{$ID}'");
		// JOIN tb_buku_tanah ON tb_buku_tanah.id_bukutanah = tb_warkah.id_bukutanah JOIN tb_hak_milik ON tb_buku_tanah.id_hak = tb_hak_milik.id_hak 
		$data = array(
			'title' => 'Dokumen Keluar Buku Tanah'.DEFAULT_TITLE, 
			'data' => $query->row(),
		);
