...
class Pinjam_buku extends CI_Controller {
	...
	public function cetak($ID=0)
	{
		$query = $this->db->query("SELECT tb_pinjam_buku.*, tb_buku_tanah.*, tb_hak_milik.*, tb_users.* FROM tb_pinjam_buku 
			INNER JOIN tb_buku_tanah ON tb_pinjam_buku.id_bukutanah = tb_buku_tanah.id_bukutanah JOIN tb_hak_milik ON tb_buku_tanah.id_hak = tb_hak_milik.id_hak JOIN tb_users ON tb_pinjam_buku.nip = tb_users.nip WHERE tb_pinjam_buku.id_pinjam_buku = '{$ID}'");
		$data = array(
			'title' => 'Dokumen Keluar Buku Tanah'.DEFAULT_TITLE, 
			'data' => $query->row(),
		);
