...
class File_buku extends CI_Controller {
	 ...
	 * @return string
	 **/
	public function delete($ID=0, $nohak=0, $id_bukutanah)
	{
		...
		redirect("buku/document/{$id_bukutanah}?t=file");
	}
	 ...
	 * @return string
	 **/
	public function bulk_action($id_bukutanah)
	{
	...
	{
		// ambil data dahulu
		$row = $this->db->query("SELECT tb_buku_tanah.*, tb_warkah.* FROM tb_buku_tanah JOIN tb_warkah ON tb_buku_tanah.no208 = tb_warkah.no208 WHERE tb_buku_tanah.id_bukutanah = '{$ID}' AND tb_buku_tanah.no_hakbuku = '{$nohak}'")->row();
        ...
        redirect("buku/document/{$row->id_bukutanah}?t=file");
	}
