...
class Cusers extends CI_Controller {
	...
	/**
	 * Mengecek Ketersediaan Nip
	 *
	 * @return string
	 **/
	public function cek()
	{
		$data = $this->db->get_where('tb_users', array('nip' => $this->input->post('nip')));
	...
	public function delete($nip=0)
	{
		$data = $this->db->get_where('tb_users', array('nip' => $this->input->post('nip')))->row();
		@unlink("./assets/user/{$data->foto}");
		$this->db->delete('tb_users', array('nip'=>$nip));
		$this->db->delete('tb_history', array('nip'=>$nip));
		redirect('setting/cusers');
	}
	...
	public function getExcel()
	{
		...
		$where = array(
           ...
           $data_history = $this->db->query("SELECT tb_history.*, tb_buku_tanah.*, tb_hak_milik.*, tb_kecamatan.*, tb_desa.*, tb_users.* FROM tb_history JOIN tb_buku_tanah ON tb_history.no_hakbuku = tb_buku_tanah.no_hakbuku JOIN tb_hak_milik ON tb_history.id_hak = tb_hak_milik.id_hak JOIN tb_kecamatan ON tb_history.id_kecamatan = tb_kecamatan.id_kecamatan JOIN tb_desa ON tb_history.id_desa = tb_desa.id_desa JOIN tb_users ON tb_history.nip = tb_users.nip WHERE tb_history.id_hak = '{$where['hak']}' AND tb_history.id_desa = '{$where['desa']}'")->result();
            $no=1; $cell=10;
            foreach($data_history as $row) :
            $objPHPExcel->setActiveSheetIndex(0)
                        ->setCellValue('A'.$cell,$no.'.')
                        ->setCellValue('B'.$cell,$row->nip)
