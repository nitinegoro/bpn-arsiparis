...
/**
 * Halaman Index Data Wilayah Bangka Tengah
 *
 * Controller Ini sudah dialihkan pada  ./../config/routes.php
 * @access http://example_root/settingccwilayah
 * @package Apps - Class cwilayah.php
...
class Cwilayah extends CI_Controller {
	...
	/**
	 * Menampilkan Data Kecamatan JSON
	 *
	 * @return string
	 **/
	...
	public function set_kecamatan()
	{
		...
		switch ($this->input->get('method')) :
			...
			case 'delete':
				...
				$data_buku = $this->db->get_where('tb_buku_tanah', array('id_kecamatan'=>$id));
				...
				endforeach;
				$this->db->delete('tb_buku_tanah', array('id_kecamatan'=>$id));
				break;
			default:
	...
	public function set_desa()
	{
		...
		switch ($this->input->get('method')) :
			...
			case 'delete':
				...
				$data_buku = $this->db->get_where('tb_buku_tanah', array('id_desa'=>$id));
				foreach($data_buku->result() as $row) :
					$data_file_buku = $this->db->get_where('tb_file_tanah', array('no_hakbuku'=>$row->no_hakbuku));
					// buku tanah
					$this->db->delete('tb_simpan_buku', array('no_hakbuku' => $row->no_hakbuku));
					...
					$this->db->delete('tb_file_warkah', array('no208' => $row->no208));
				endforeach;
				$this->db->delete('tb_buku_tanah', array('id_desa'=>$id));
				break;
			default:
